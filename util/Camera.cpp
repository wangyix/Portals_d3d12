...
void Camera::Level()
{
  XMVECTOR BU = XMVectorSet(0.0f, 1.0f, 0.0f, 0.0f);
	XMVECTOR L = XMLoadFloat3(&Look);
	...
	{
    R = XMVectorSet(1.0f, 0.0f, 0.0f, 0.0f);
	}
	else
	{
...
XMFLOAT3 Camera::SelfVirtualCollision(const XMMATRIX &Virtualize, float SphereRadius, XMFLOAT3 S,
    XMFLOAT3 Dir, float MoveDist, float *XDist_ptr, float *RedirectRatio_ptr,
    XMFLOAT3 *RedirectDir_ptr)const
{
	// defaults
	*XDist_ptr = MoveDist;
